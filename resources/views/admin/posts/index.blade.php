                ...
                <tr class="post-row" data-user="{{ $post->user->business_name }}" data-date="{{ $post->upload_date->format('Y-m-d') }}" data-platform="{{ strtolower($post->platform) }}">
                    <td class="p-3 ps-4 border-0">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="me-3 position-relative shadow-sm" style="width: 40px; height: 55px; border-radius: 8px; overflow: hidden; cursor: pointer; background: #f1f5f9; flex-shrink: 0;" onclick="window.open('{{ $post->post_url }}', '_blank')" title="Buka Postingan Asli">
                                <div style="position: absolute; inset: 0; z-index: 10;"></div>
                                
                                @if(str_contains($post->cover_image, '<blockquote') || str_contains($post->cover_image, '<iframe'))
                                    <div class="d-flex align-items-center justify-content-center w-100 h-100" style="background: #1e293b; color: #fff;">
                                        <i class="bi bi-play-fill fs-3"></i>
                                    </div>
                                @else
                                    <img src="{{ asset('storage/'.$post->cover_image) }}" style="width: 100%; height: 100%; object-fit: cover;">
                                @endif
                            </div>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 24px; overflow: hidden;">
            <div class="modal-body p-0 bg-white">
                <div class="row g-0">
                    {{-- DIUBAH: Diberi padding dan tinggi yang cukup agar iframe TikTok (734px) masuk sempurna --}}
                    <div id="media-container" class="col-md-5 d-flex align-items-center justify-content-center bg-light py-4 px-2" style="min-height: 750px; max-height: 800px; overflow-y: auto;">
                        <img id="detail-cover" src="" class="shadow rounded-4 w-100" style="max-height: 580px; object-fit: contain;">
                    </div>
                    <div class="col-md-7 p-4">
                        <div class="d-flex justify-content-between align-items-start mb-4">
                            <div>
                                <h4 id="detail-mitra" class="fw-900 text-dark mb-1"></h4>
                                <span id="detail-platform-badge" class="badge text-uppercase mb-2" style="border-radius: 6px; padding: 6px 12px; font-weight: 700; background: var(--teal-primary); color: white;"></span>
                        ...
                        <div id="detail-extra-container" class="p-3 rounded-4 border bg-white mb-4">
                            <div id="detail-watch-time" class="mb-3 d-none border-bottom pb-2">
                                <small class="text-muted fw-bold d-block mb-1" style="font-size: 10px;">RATA-RATA LAMA MENONTON</small>
                                <span class="fw-800 text-dark fs-5" id="val-watch-time"></span>
                            </div>
                            <small class="text-muted fw-bold d-block mb-2" style="font-size: 10px;">DEMOGRAFI USIA</small>
                            <div id="detail-age-list" class="d-flex flex-wrap gap-2"></div>
                        </div>

                        <a id="detail-link" href="#" target="_blank" class="btn btn-dark w-100 fw-bold py-3" style="border-radius: 12px;"><i class="bi bi-box-arrow-up-right me-2"></i> Kunjungi Postingan</a>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function showDetailFromTable(p, name) {
        ...
        grid.innerHTML = `
            <div class="col-6"><div class="p-3 rounded-4 border bg-light text-center"><small class="text-muted fw-bold">VIEWS</small><h4 class="fw-800 mb-0">${p.views.toLocaleString()}</h4></div></div>
            <div class="col-6"><div class="p-3 rounded-4 border bg-light text-center"><small class="text-muted fw-bold">LIKES</small><h4 class="fw-800 mb-0">${p.likes.toLocaleString()}</h4></div></div>
            <div class="col-6"><div class="p-3 rounded-4 border bg-light text-center"><small class="text-muted fw-bold">COMMENTS</small><h4 class="fw-800 mb-0">${p.comments.toLocaleString()}</h4></div></div>
            <div class="col-6"><div class="p-3 rounded-4 border bg-light text-center"><small class="text-muted fw-bold">SHARES</small><h4 class="fw-800 mb-0">${p.shares.toLocaleString()}</h4></div></div>
        `;
        
        const mediaContainer = document.getElementById('media-container');
        const modalEl = document.getElementById('modalDetail');

        // LOGIKA BARU: Ekstraksi ID agar Iframe Rapi & Tombol Play Lancar!
        if (p.cover_image && (p.cover_image.includes('<blockquote') || p.cover_image.includes('<iframe'))) {
            
            if (p.cover_image.includes('tiktok')) {
                // Ekstrak ID Video dari teks embed
                let videoId = null;
                let match = p.cover_image.match(/data-video-id=["']?(\d+)["']?/);
                if (match) videoId = match[1];
                if (!videoId) {
                    match = p.cover_image.match(/video\/(\d+)/);
                    if (match) videoId = match[1];
                }

                if (videoId) {
                    // DIUBAH: Height diset ke 734px (tinggi original TikTok embed) agar sensor tombol play tidak meleset
                    // Dihapus parameter autoplay agar sistem keamanan browser/tiktok tidak nge-lock player-nya
                    mediaContainer.innerHTML = `<iframe src="https://www.tiktok.com/embed/v2/${videoId}" style="width: 100%; max-width: 325px; height: 734px; border: none; border-radius: 16px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); display: block; margin: 0 auto;" allow="autoplay; fullscreen; encrypted-media; picture-in-picture;" allowfullscreen scrolling="no"></iframe>`;
                } else {
                    mediaContainer.innerHTML = `<div class="w-100 d-flex justify-content-center" style="max-height:734px; overflow-y:auto; overflow-x:hidden;">${p.cover_image}</div>`;
                }
            } else {
                mediaContainer.innerHTML = `<div class="w-100 d-flex justify-content-center" style="max-height:734px; overflow-y:auto; overflow-x:hidden;">${p.cover_image}</div>`;
                if (p.cover_image.includes('instagram') && window.instgrm) {
                    setTimeout(() => window.instgrm.Embeds.process(), 200);
                }
            }

        } else {
            mediaContainer.innerHTML = `<img id="detail-cover" src="${window.location.origin + '/storage/' + p.cover_image}" class="shadow rounded-4 w-100" style="max-height: 580px; object-fit: contain;">`;
        }
    ...
    document.addEventListener('DOMContentLoaded', () => setQuickRange(7));
</script>
@endsection